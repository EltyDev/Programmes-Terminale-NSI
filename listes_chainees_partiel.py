...
def ajouter(tab, element):
    ...
    # Et donc on crée le nouveau tableau avec la valeur à la place de "None"
    next[1] = [element, None]
...
def supprimer_ind(tab, index):
    next = tab  # On lui donne la valeur de la liste
    # On continue jusqu'à l'index-1 donné pour avoir le tableau juste avant celui de l'index
